...
public class Trade {
	...
	public static ItemStack submit = ItemUtils.getItem(Material.WOOL, (short) 5, "&a&lSubmit");
	public static ItemStack submited = ItemUtils.getItem(Material.WOOL, (short) 5, "&a&lSubmited", "this player has", "accepted the trade");
	public static ItemStack unsubmited = ItemUtils.getItem(Material.WOOL, (short) 14, "&c&oUnsubmited", "waiting confirmation");
	...
	public static void display(Player p) {
		...
		for (int n = 0; n < 6; n++) inv.setItem(n*9+4, ItemUtils.ItemDefault());
		inv.setItem(3, submit);
		inv.setItem(5, unsubmited);
		
		Inventory bag = Bag.inventories.get(p);
		for (int n = 0; n < 27; n++) p.getInventory().setItem(n+9, bag.getItem(n));
		
		p.updateInventory();
		p.openInventory(inv);
	}
	
	public static String displayN() { return ChatColor.translateAlternateColorCodes('&', "&aTrad &2o &amatic &62000"); }
	
	public static void submit(Player p) {
		if (!p.getOpenInventory().getItem(5).equals(submited)) {
			Player other = TradeCommand.pending.get(p);
			p.getOpenInventory().setItem(3, Functions.addGlow(p.getOpenInventory().getItem(3).clone()));
			p.updateInventory();
	...
	private static Player remove(Player p, boolean trade) {
		...
		Integer[] slots = { 0, 1, 2, 9, 10, 11, 12, 18, 19, 20, 21, 27, 28, 29, 30, 36, 37, 38, 39, 45, 46, 47, 48 };
		for (int slot : slots) {
			ItemStack i1 = p.getOpenInventory().getItem(slot);
			ItemStack i2 = other.getOpenInventory().getItem(slot);
			if (i1 != null) if (trade) other.getInventory().addItem(i1); else p.getInventory().addItem(i1);
			if (i2 != null) if (trade) p.getInventory().addItem(i2); else other.getInventory().addItem(i2);
		}
		for (int n = 0; n < 27; n++) {
			Bag.inventories.get(p).setItem(n, p.getInventory().getItem(n+9));
			Bag.inventories.get(other).setItem(n, other.getInventory().getItem(n+9));
