	...
	public static void open(Player p) {
		...
		if (PlayerConfig.get(p).contains("Pets")) {
			...
			if (ls.contains(Pets.Wolf.getType()))
				if (InventoryClick.petEnum.containsKey(p)
						&& InventoryClick.petEnum.get(p).equals(Pets.Wolf))
					p.getInventory().setItem(15,
							Functions.addGlow(ItemUtils.Wolf().clone()));
				else
					p.getInventory().setItem(15, ItemUtils.Wolf());
			if (ls.contains(Pets.Cat.getType()))
				if (InventoryClick.petEnum.containsKey(p)
						&& InventoryClick.petEnum.get(p).equals(Pets.Cat))
					p.getInventory().setItem(16,
							Functions.addGlow(ItemUtils.Cat().clone()));
				else
					p.getInventory().setItem(16, ItemUtils.Cat());
			p.updateInventory();
		}
