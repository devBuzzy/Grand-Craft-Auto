...
public class PetStore implements BasicStore, Confirmable {
	...
	public static ItemStack wolf = ItemUtils.getItem(Material.MONSTER_EGG,
			(short) 95, "&a&lDoggy Friend", "&bJust a dog",
			"&6&lCost: &d500 Coins");
	public static ItemStack cat = ItemUtils.getItem(Material.MONSTER_EGG,
			(short) 98, "&6&lNian Style", "&d&oMeeew", "&6&lCost: &d750 Coins");
	public static String ShopN = ChatColor.translateAlternateColorCodes('&',
			"&a&lPet Store");
	public static HashMap<Player, Boolean> confirming = new HashMap<Player, Boolean>();

	public String welcome() {
		return ChatColor.translateAlternateColorCodes('&',
				"&aPet Clerk &d-> &eWelcome!");
	}
		...
		JsonMessage.sendJson(
				...
				JsonMessage
						...
						.hoverEvent(HoverAction.Show_Text,
								"&bPurchase an animal")
						.clickEvent(ClickAction.Run_Command, "/Store Pet")
						.build().build(),
				JsonMessage
						.newJson()
						.add("          &4&l*")
	...
	public static void Buy(Player p, Pets pet) {
		...
		Messager.load(p);
		if (!fc.contains("Pets"))
			fc.set("Pets", new ArrayList<String>());
		List<String> ls = fc.getStringList("Pets");
		if (ls.contains(pet.getType())) {
			Messager.e1("You already have that pet");
			p.closeInventory();
		} else {
	...
	public void yes(Player p) {
		...
		new Chat(p).msg("&a&lName your pet", "  &2&oType a name in chat!",
				"    &c&oWarning: You can`t change it",
				"&dColors not supported!");
	}
