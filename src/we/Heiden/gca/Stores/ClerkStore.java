...
public class ClerkStore implements BasicStore, Confirmable, Displayable {
	...
	public String welcome() { return ChatColor.translateAlternateColorCodes('&', "&a&lClerk &d-> &b25% Off! &2(just kidding)"); }
		...
		JsonMessage.sendJson(p, 
				...
				JsonMessage.newJson()
					...
					.add("&a&lPurchase")
						...
						.clickEvent(ClickAction.Run_Command, "/Store Clerk").build().build(), 
	...
	public void display(Player p) {
		...
		inv.setItem(3, ItemUtils.Food());
		inv.setItem(5, ItemUtils.Weapons());
		
		p.openInventory(inv);
	}
	
	public static String displayN = ChatColor.translateAlternateColorCodes('&', "&a&lClerk Shop");
	...
	public static String weaponsN = ChatColor.translateAlternateColorCodes('&', "&c&lWeapons &bStore");
	...
	public static String foodN = ChatColor.translateAlternateColorCodes('&', "&a&lFood n Heal");
	...
	public static void confirm(Player p, ItemStack item, int price) {
		...
		inv.setItem(4, ItemUtils.getItem(Material.DOUBLE_PLANT, "&6&lPrice: &e" + price + " coins"));
