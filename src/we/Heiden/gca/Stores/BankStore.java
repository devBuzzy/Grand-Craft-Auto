...
public class BankStore implements BasicStore {
	...
	public String welcome() {
		return ChatColor.translateAlternateColorCodes('&',
				"&aDealership Owner &d-> &eWelcome!");
	}

	public void options(Player p) {
		int bankMoney = 0;
		if (PlayerConfig.get(p).contains("Bank"))
			bankMoney = PlayerConfig.get().getInt("Bank");
				...
				.sendJson(
						...
						JsonMessage
								...
								.hoverEvent(HoverAction.Show_Text,
										"&9&oWithdraw money")
								.clickEvent(ClickAction.Run_Command,
										"/Store Bank Withdraw").build().build(),
						JsonMessage
								.newJson()
								.add("     &6&l··&b&l> ")
								.build()
								...
								.hoverEvent(
										...
										(Functions.canJoinRobbery() ? "&d&l•• &6&l►► &a&lEVENT OPEN &6&l◄◄ &d&l••"
												: "&cYou can join from 8:00 PM to 8:05 PM"))
								.clickEvent(ClickAction.Run_Command,
										"/Store Robbery Bank").build()
	...
	public static void withdraw(Player p, int amount) {
		...
		if (PlayerConfig.get(p).contains("Bank"))
			bankMoney = PlayerConfig.get().getInt("Bank");
		if (bankMoney < amount)
			new Chat(p).e("You don`t have enough Money");
		else {
			bankMoney -= amount;
