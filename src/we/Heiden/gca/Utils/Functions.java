...
public class Functions {
	...
	public static double getBankMoney(Player p) {
		if (PlayerConfig.get(p).contains("Bank"))
			return PlayerConfig.get().getDouble("Bank");
		return 0;
	}

	public static void setBankMoney(Player p, double money) {
		PlayerConfig.get(p).set("Bank", money);
		PlayerConfig.save();
	}

	public static Player hide(Player p) {
	...
	public static boolean displayEffect(Location loc, int id, float velocity,
			int amount, List<Player> player) {
		Player[] pl = {};
		return displayEffect(loc, id, velocity, amount, player.toArray(pl));
	}

	public static boolean displayEffect(Location loc, int id, float velocity,
			int amount, Player... pl) {
		try {
			Constructor<?> packetConstructor = PacketPlayOutWorldParticles.class
					.getConstructor();
	...
	public static boolean isInt(String s) {
		return isInt(null, s, null, true, true, false);
	}

	public static boolean isInt(String s, boolean zero, boolean negative) {
		return isInt(null, s, null, zero, negative, false);
	}
	...
	public static void saveLoc(String path, Location loc, Player p) {
		if (p == null || canFc(p))
			saveLoc(path, loc, fc);
	}

	public static void saveLoc(String path, Player p) {
		if (canFc(p))
			saveLoc(path, p.getLocation(), fc);
	}

	public static void saveLoc(String path, Player p, FileConfiguration fc) {
		saveLoc(path, p.getLocation(), fc);
	}

	public static Location loadLoc(String path, Player p) {
		if (canFc(p))
			return loadLoc(path, fc);
		else
			return null;
	}
	...
	public static boolean isOnArea(Location p1, Location p2, Location loc) {
		...
		if (x > xM || y > yM || z > zM || x < xm || y < ym || z < zm)
			return false;
		return true;
	}

	public static boolean isOnArea(Location p1, Location p2, Player p) {
		return isOnArea(p1, p2, p.getLocation());
	}
}
